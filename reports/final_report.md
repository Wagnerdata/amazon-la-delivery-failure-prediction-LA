...
**Date:** April 2026  
**Model:** Random Forest + SMOTE | **AUC-ROC:** 0.7986 | **Recall @ 0.05:** 87.5%

---

## 0. Through the Process: The Project Journey (Oct 2025 – April 2026)

This project represents a six-month analytical evolution, transitioning from academic discovery to a deployable operational tool for Amazon LA.

### Milestone 1: Discovery & Strategy (Oct – Dec 2025)
*   **Tooling**: Started in **Jupyter Notebooks** to allow for rapid iteration and visual debugging.
*   **Decisions**: Chose the Amazon LMRC 2018 dataset due to its real-world noise. Initially, I focused on simple logistic regression, but the "Urban Density Paradox" discovery in November 2025 forced a pivot to tree-based ensembles (**Random Forest**) to capture non-linear interactions.

### Milestone 2: Development & Refinement (Jan – Mar 2026)
*   **Optimization**: Implemented **SMOTE** to handle the severe $140:1$ class imbalance discovered during early training.
*   **Refactoring**: Transitioned core logic from "scratchpad" notebooks to a professional Python codebase and began drafting the Word documentation.

### Milestone 3: Packaging & Deployment (April 2026)
*   **Deliverables**: Finalized all five documentation pillars (Deliverables 1-5).
*   **Dashboard**: Deployed the interactive **Streamlit** application to bring the "Pre-dispatch Scoring" concept to life for non-technical stakeholders.
...
|---|---|---|
| **AUC-ROC** | **0.7986** | Strong discrimination — random = 0.5, perfect = 1.0 |
| **Recall @ 0.05** | **87.5%** | 87.5% of actual failures flagged at threshold 0.05 |
...
| Recall (Failed) | 0.875 | Model captures 87.5% of actual failures at threshold 0.05 |
...
- True Positives: ~22 (87.5% of 25 actual failures caught)
...
An AUC of 0.7986 means the model correctly ranks a randomly selected failed package as higher-risk than a successful package ~80% of the time. For a pre-dispatch screening tool where the goal is to flag high-risk packages for review, this discrimination ability translates to meaningful operational value — the model's top-scoring deciles capture a disproportionate share of actual failures.
...
3. **The SMOTE+RF model achieves AUC-ROC of 0.7986 with 87.5% recall at threshold 0.05** — catching 87.5% of actual failures at dispatch time, enabling proactive intervention before packages leave the facility.
